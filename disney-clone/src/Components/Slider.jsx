...
function Slider() {
  ...
  return (
    ...
    <div className='flex overflow-x-auto min-w-full px-16 py-4 scrollbar-none scroll-smooth' ref={elementRef}>
      {movieList.map((item,index) => (
        <img src={`${ImageBaseUrl}${item.backdrop_path}`}
         className='min-w-full md:h-[300px] object-cover object-left-top mr-10 rounded-md hover:border-4 hover:border-gray-400 transition-all duration-75 ease-in' alt=""  />
      ))}
    </div>
    </div>
  )
